...
- **The resampling executor is selectable (F013).** `F1_GRID_EXECUTOR` picks `pandas` (the default, and the engine F010's validation runs on) or `spark`. The Spark path is the same `resample_lap` inside a grouped-map pandas UDF against a Spark Connect server in the `pipeline` profile, and it writes the same files: both executors share `finish_session`, so equivalence is a property of the code rather than a hope. The Spark image is built from `python:3.12-slim` plus `pyspark==4.1.1`, not `apache/spark`, because that image's Python 3.10 cannot install this project's pins and UDF bytecode does not cross Python minors (D2, amended).
...
- **The distance axis has a placed origin (F015).** Distance zero is not the reference line's own start — it is moved to the latest telemetry opening in the session, so that every lap has a real sample at or before grid point zero. Without it the resampler extrapolates the time channel across a lap's leading grid points, which is a silent per-lap bias (−0.19 s at Bahrain) and a non-increasing time channel that fails the F011 gate. `grid_index` is only comparable within a session, and only under this convention.
- **Contracts describe the sport, not one session type.** F011's vocabularies were derived from a qualifying session and were wrong for races: DRS codes 2, 3, 13 and 15 all appear in race telemetry, and throttle reaches 104 %. A contract that fails a whole session over the source's own vocabulary is a contract bug. A genuine glitch — 49 samples reporting a gear of 72 — is tolerated by `max_fraction` instead, because global §3.1 says a corrupt record is skipped rather than stopping the batch.
