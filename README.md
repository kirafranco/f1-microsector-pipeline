...
Everything persists in bind mounts under `data/` (gitignored), so copying the project folder carries the databases with it. Which host interface each port is published on is a per-machine setting in `.env`. By default both bind to every interface, Postgres on port 55432 and Grafana on 3000, so the database can be queried and the dashboard opened from other devices on the local network; a machine that should keep either local sets the corresponding `*_BIND_ADDRESS` to `127.0.0.1`. Nothing is exposed beyond the local network, and every connection needs the generated credentials. Grafana connects through a read-only role and its datasource and dashboards are provisioned from files in `servicios/grafana/provisioning/`. The standards the compose file must obey are asserted by tests that run without Docker; `pytest -m docker` brings the stack up and checks health, permissions, persistence and limits on the real daemon.
